...
local config = require("nvim-possession.options")
local commands = require("nvim-possession.commands")
...
M.setup = function(opts)
  ...
  if config.options.autosave == true then
    vim.api.nvim_create_autocmd("VimLeavePre", {
      group = vim.api.nvim_create_augroup("NvimPossessionAutoSave", { clear = true }),
      callback = function()
        require("nvim-possession.core").update({ ask_confirmation = false })
      end,
    })
  end
...
do
  ...
  for _, v in pairs(available_commands) do
    M[v] = require("nvim-possession.core")[v]
  end
end
